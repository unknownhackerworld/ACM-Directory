...
export default function MemberProfile() {
  ...
  if (!member) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-white p-4">
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-white flex flex-col">
      ...
      <main className="flex-grow">
        <div className="max-w-5xl mx-auto py-8 md:py-16 px-4 sm:px-6 lg:px-8 xl:px-20">
          ...
          <button
            onClick={() => navigate('/')}
            className="inline-flex items-center gap-2 md:gap-3 border border-gray-400 dark:border-gray-600 text-gray-800 dark:text-white px-4 py-2 md:px-6 md:py-3 rounded-xl mb-6 md:mb-10 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-200 text-base md:text-xl"
          >
          ...
          {/* Profile Card */}
          <div className="bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl shadow-lg md:shadow-2xl p-6 sm:p-8 md:p-12 lg:p-16 xl:p-20 grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-10">
            {/* Left Section */}
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left space-y-4 md:space-y-6">
              <div className="relative w-32 h-32 md:w-40 md:h-40">
                ...
                <img
                  src={avatarUrl}
                  alt="Profile"
                  className={`w-full h-full rounded-full shadow-md md:shadow-xl border-4 border-blue-100 dark:border-blue-900 object-cover transition-opacity duration-300 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
              ...
              <div className="space-y-1 md:space-y-2">
                ...
                <p className="text-lg md:text-xl lg:text-2xl text-blue-700 dark:text-blue-300 font-medium">{member.role}</p>
                <p className="text-base md:text-lg lg:text-xl text-gray-600 dark:text-gray-300">{member.domain}</p>
              </div>
            </div>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-2 md:gap-3 mt-1 md:mt-2">
                  {member.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-3 py-1 md:px-4 md:py-1 rounded-full text-sm md:text-base"
                    >
                      {skill}
                    </span>
                  ))}
      ...
      <footer className="bg-blue-900 dark:bg-gray-800 text-white text-center py-4 md:py-6 mt-6 md:mt-10">
